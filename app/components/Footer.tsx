...
export default function Footer() {
  ...
  return (
    <footer className="bg-background pt-24 pb-12 px-6 border-t border-border relative overflow-hidden transition-colors duration-500">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12 mb-20">
          <div className="lg:col-span-2">
            ...
            <h3 className="text-4xl font-black mb-6 tracking-tighter uppercase italic text-foreground">
            ...
            <div className="flex gap-4">
              {[FiLinkedin, FiGithub, FiInstagram].map((Icon, i) => (
                <motion.a
                  key={i}
                  // whileHover background uses primary, color uses background to ensure visibility
                  whileHover={{ y: -8, backgroundColor: 'var(--color-primary)', color: 'var(--background)' }}
                  href="#"
                  className="w-12 h-12 rounded-2xl bg-card border border-border flex items-center justify-center text-xl text-foreground transition-all shadow-lg"
                >
                  <Icon />
                </motion.a>
              ))}
          ...
          <div>
            <h4 className="font-black uppercase tracking-[0.2em] text-[10px] mb-8 text-primary">Quick Links</h4>
          ...
          <div className="flex flex-col justify-between items-start md:items-end">
            <motion.button
              ...
              whileTap={{ scale: 0.9 }}
              onClick={scrollToTop}
              // bg-primary and text-background ensures icon color flips based on theme
              className="w-16 h-16 bg-primary text-background rounded-3xl flex items-center justify-center text-2xlshadow-lg shadow-primary/20"
            >
              <FiArrowUp strokeWidth={3} />
            </motion.button>
            <div className="text-left md:text-right mt-12 md:mt-0 text-foreground">
        ...
        {/* Bottom Bar */}
        <div className="pt-8 border-t border-border flex flex-col md:flex-row justify-between items-center gap-6 text-[10px] font-black uppercase tracking-[0.2em] text-foreground/40">
